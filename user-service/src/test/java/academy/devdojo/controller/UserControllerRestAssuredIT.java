...
class UserControllerRestAssuredIT extends IntegrationTestConfig {
    ...
    @Autowired
    @Qualifier("requestSpecificationAdminUser")
    ...
    void update_UpdatesUser_WhenSuccessful() throws Exception {
        ...
        List<User> users = repository.findByFirstNameIgnoreCase("Tanjirou");
        ...
        User updatedUser = repository.findById(oldUser.getId()).orElseThrow(() -> new NotFoundException("User Not Found"));
        String encryptedPassword = updatedUser.getPassword();
        Assertions.assertThat(passwordEncoder.matches("strongest", encryptedPassword)).isTrue();
    }
